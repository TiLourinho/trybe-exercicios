...
timer("Números carregados na memória secundária", secondary_load_initial_time)  # noqa

secondary_get_init_time = perf_counter()
secondary_sum = sum([secondary.get(i) for i in range(len(RANDOM_NUMBERS))])
timer(f"Soma na memória secundária: {secondary_sum}", secondary_get_init_time)  # noqa
