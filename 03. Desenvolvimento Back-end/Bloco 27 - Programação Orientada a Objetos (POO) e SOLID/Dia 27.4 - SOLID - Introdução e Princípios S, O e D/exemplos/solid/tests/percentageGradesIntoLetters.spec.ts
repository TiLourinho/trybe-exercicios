...
describe('Testando a função "percentageGradesIntoLetters"', function () {
  describe('quando é passado um array de disciplinas válidas', function () {
    ...
    const resultDisciplines = percentageGradesIntoLetters(disciplines) as Discipline;

    const expectedGrades = ['A', 'B', 'C', 'D', 'E', 'F'];
    const givenGrades = resultDisciplines.map((discipline: { letterGrade: any; }) => discipline.letterGrade);
